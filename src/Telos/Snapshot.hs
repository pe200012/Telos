...
data Snapshot
  = Snapshot { _snapHash  :: Text        -- ^ Git tree hash at this point
...
data SnapshotConfig
  = SnapshotConfig { _scEnabled     :: Bool        -- ^ Whether snapshots are enabled
  ...
  if enabled
    then do
      ...
      case result of
        ...
        Right gitDir -> pure $ SnapshotConfig True projectPath gitDir
    else pure $ SnapshotConfig False projectPath ""
...
runGit gitDir workTree args = do
  ...
  let gitEnv = [ ( "GIT_DIR", gitDir ), ( "GIT_WORK_TREE", workTree ) ] <> existingEnv
