...
import           Telos.Core.Types    ( Message(..), amContent, amToolCalls, tcArguments, tcName )
...
transformWithId pstate idx msg = case msg of
  ToolResultMessage callId toolName _result isErr -> let
      rid = ToolResultId idx
    in 
...
transformMessage pstate rid msg = case msg of
  ToolResultMessage callId toolName _result isErr -> if rid `Set.member` (pstate ^. psMarkedIds)
    then case Map.lookup rid (pstate ^. psDistillations) of
      Just distill -> ToolResultMessage callId toolName distill isErr
      Nothing      -> ToolResultMessage callId toolName outputRemovedPlaceholder isErr
    else msg
...
formatPrunableList infos
  ...
  where
    formatInfo info
      = T.pack (show $ _unToolResultId $ info ^. piId)
      <> ": "
      <> info ^. piToolName
      <> ", "
      <> info ^. piParams
...
injectPrunableList config pstate msgs
  = let
      infos = getPrunableInfos config pstate
    in 
      if null infos
        then msgs
...
buildToolCache = snd . foldl' processMessage ( 0, Map.empty )
  where
    processMessage :: ( Int, Map ToolResultId ToolCacheEntry )
                   -> Message
                   -> ( Int, Map ToolResultId ToolCacheEntry )
    processMessage ( turnIdx, cache ) msg = case msg of
      UserMessage _ -> ( turnIdx + 1, cache )  -- New turn on user message
      ToolResultMessage _callId toolName result isErr -> let
          rid   = ToolResultId (Map.size cache)
          entry
            = ToolCacheEntry
        ...
        in 
          ( turnIdx, Map.insert rid entry cache )
      _ -> ( turnIdx, cache )
...
updateToolCache pstate toolName paramKey isErr mFilePath result
  = let
      nextId = ToolResultId (Map.size $ pstate ^. psToolCache)
      entry
        = ToolCacheEntry
        { _tceToolName      = toolName
    ...
    in 
      pstate & psToolCache %~ Map.insert nextId entry
...
estimateMessageTokens msg = case msg of
  ...
  AssistantMsg am -> maybe 0 estimateTokens (am ^. amContent)
    + 4
    + sum
      [ estimateTokens (tc ^. tcName) + estimateTokens (encodeArgs $ tc ^. tcArguments)
      | tc <- am ^. amToolCalls
      ]
