...
data PrunableInfo
  = PrunableInfo { _piId       :: ToolResultId  -- ^ Unique ID
...
data PruneStats
  = PruneStats { _psTotalPruned :: Int              -- ^ Total items pruned
...
emptyPruneStats = PruneStats { _psTotalPruned = 0, _psTokensSaved = 0, _psByStrategy = Map.empty }

-- | Pruning state per session
data PruneState
  = PruneState { _psMarkedIds     :: Set ToolResultId              -- ^ IDs marked for pruning
...
emptyPruneState
  = PruneState { _psMarkedIds     = Set.empty
...
data PruneConfig
  = PruneConfig { _pcEnabled        :: Bool      -- ^ Whether DCP is enabled
