...
runStrategies config pstate
  ...
  | otherwise
    = foldl'
      (\s strategy -> strategy config s)
      pstate
      [ deduplicationStrategy, supersedeWritesStrategy, purgeErrorsStrategy ]
...
deduplicationStrategy config pstate
  = let
      ...
      groupByKey :: ToolResultId
                 -> ToolCacheEntry
                 -> Map Text [ ( ToolResultId, ToolCacheEntry ) ]
                 -> Map Text [ ( ToolResultId, ToolCacheEntry ) ]
      groupByKey rid entry acc
        = let
            k = entry ^. tceParamKey
          in 
            Map.insertWith (++) k [ ( rid, entry ) ] acc
      ...
      findOlderDuplicates entries
        ...
        | otherwise
          = let
              sorted = sortOn (negate . (^. tceTurnIndex) . snd) entries  -- newest first
            in 
              case sorted of
                (_newest : older)
                  -> mapMaybe (filterPrunable currentTurn protectedTurns protectedTools) older
                [] -> []

      filterPrunable
        :: Int -> Int -> Set Text -> ( ToolResultId, ToolCacheEntry ) -> Maybe ToolResultId
      filterPrunable curTurn protection protected ( rid, entry )
        ...
        | otherwise = Just rid
    in 
      pstate & psMarkedIds %~ Set.union idsToMark
...
supersedeWritesStrategy config pstate
  = let
      ...
      groupByFile :: ToolResultId
                  -> ToolCacheEntry
                  -> Map FilePath [ ( ToolResultId, ToolCacheEntry ) ]
                  -> Map FilePath [ ( ToolResultId, ToolCacheEntry ) ]
      ...
      findSupersededWrites entries
        = let
            sorted         = sortOn ((^. tceTurnIndex) . snd) entries  -- oldest first
            ...
            latestReadTurn = case readOps of
              [] -> -1
              _  -> foldr (max . (^. tceTurnIndex) . snd) (-1) readOps
          in 
            [ rid
            | ( rid, entry ) <- writes
...
extractFilePath toolName params
  | toolName `elem` [ "read", "write", "edit" ] = toString <$> (params ^? key "filePath" . _String)
  | toolName == "bash" = toString <$> (params ^? key "workdir" . _String)
  | otherwise = Nothing
...
computeParamKey toolName params
  = let
      paramBytes = toStrict $ encode params
