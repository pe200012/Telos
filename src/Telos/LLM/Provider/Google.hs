...
newProvider model config manager = do
  case config ^. pcApiKey of
    ...
    Just apiKey -> do
      let baseUrl = config ^. pcBaseURL . non "https://generativelanguage.googleapis.com/v1beta"
...
convertToGeminiMessages msgs
  = let
      systemPrompt = listToMaybe [ c | SystemMessage c <- msgs ]
      contents     = flip mapMaybe msgs $ \case
        UserMessage c -> Just
          $ A.object [ "role" A..= ("user" :: Text), "parts" A..= [ A.object [ "text" A..= c ] ] ]
        AssistantMsg am -> Just
          $ A.object
            [ "role" A..= ("model" :: Text)
            , "parts" A..= [ A.object [ "text" A..= (view amContent am ^. non "") ] ]
            ]
        SystemMessage _ -> Nothing  -- Extracted separately
        ToolResultMessage _ name result _ -> Just
          $ A.object
            [ "role" A..= ("user" :: Text)
            , "parts"
              A..= [ A.object
                       [ "functionResponse"
                         A..= A.object
                           [ "name" A..= name, "response" A..= A.object [ "result" A..= result ] ]
                       ]
                   ]
            ]
    in
...
convertToGeminiTools tools = A.object [ "function_declarations" A..= map convertTool tools ]
  where
    convertTool tool
      = A.object
...
buildRequest baseUrl apiKey model messages tools streaming = do
      ...
      body
        ...
        $ catMaybes
          [ ("system_instruction" A..=)
            . (\txt -> A.object [ "parts" A..= [ A.object [ "text" A..= txt ] ] ])
            <$> systemPrompt
          , Just $ "contents" A..= geminiContents
          , if null tools
              then Nothing
              else Just $ "tools" A..= [ convertToGeminiTools tools ]
          ]
  ...
  pure
    ...
    { method         = "POST"
    , requestHeaders
        = [ ( "x-goog-api-key", TE.encodeUtf8 apiKey ), ( "content-type", "application/json" ) ]
...
parseGeminiResponse val = do
  ...
  let textBlocks = flip mapMaybe (toList parts) $ \case
  ...
  where
    parseLine line
      | "data: " `BS8.isPrefixOf` line
        = let
            jsonPart = BS.drop 6 line
          in
            maybeToList (A.decodeStrict jsonPart)
      | otherwise = []
