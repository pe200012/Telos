  ...
  case config ^. pcApiKey of
    ...
    Just apiKey -> do
      let baseUrl = case config ^. pcBaseURL of
            Just url -> url
            Nothing  -> "https://api.mistral.ai/v1"
  ...
  case result of
    ...
    Right resp -> case statusCode (responseStatus resp) of
      200 -> case A.eitherDecode (responseBody resp) of
        ...
        Right (chatResp :: CopilotClient.ChatResponse) -> case chatResp ^. CopilotClient.chChoices of
...
buildRequest baseUrl apiKey model messages tools stream = do
  let reqBody = A.object
        [ "model" A..= model
        , "messages" A..= messages
        , "tools" A..= (if null tools then A.Null else A.toJSON $ map wrapTool tools)
        , "stream" A..= stream
        ]
      ...
      (choice:_) -> case choice ^. CopilotClient.chDelta of
        ...
        Just delta -> do
          case delta ^. CopilotClient.dContent of
            ...
            Nothing -> pure ()
          case delta ^. CopilotClient.dToolCalls of
            Just tcs -> mapM_ processToolCall tcs
            Nothing -> pure ()
