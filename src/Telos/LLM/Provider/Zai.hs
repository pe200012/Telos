...
import           Control.Lens                ( (^.), (^?), _Just, at, ix, non, view )
...
newProvider model config manager = do
  case config ^. pcApiKey of
    ...
    Just apiKey -> do
      let baseUrl = config ^. pcBaseURL . non "https://api.z.ai/v1"
...
buildRequest baseUrl apiKey model messages tools streaming = do
  let url        = T.unpack baseUrl <> "/chat/completions"
      toolsField = ([ "tools" A..= (convertTools <$> tools) | not (null tools) ])
      body
        = A.object
        $ [ "model" A..= model
  ...
  pure
    $ initReq { method         = "POST"
              , requestHeaders = [ ( "Authorization", "Bearer " <> TE.encodeUtf8 apiKey )
...
parseStreamChunks = awaitForever $ \case
  ...
  where
    ...
    processToolCall = \case
      A.Object tcObj -> case KM.lookup "type" tcObj of
        Just (A.String "function") -> case extractToolCallInfo (A.Object tcObj) of
          Just ( idx, args ) -> yield $ ToolCallDelta idx args
          _ -> pure ()
        _ -> pure ()
      _ -> pure ()
