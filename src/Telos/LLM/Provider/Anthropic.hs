...
newProvider model config manager = do
  case config ^. pcApiKey of
    ...
    Just apiKey -> do
      let baseUrl = config ^. pcBaseURL . non "https://api.anthropic.com/v1"
...
convertToAnthropicMessages msgs
  = let
      ...
      messages     = flip mapMaybe msgs $ \case
        UserMessage c -> Just $ A.object [ "role" A..= ("user" :: Text), "content" A..= c ]
        AssistantMsg am -> Just
          $ A.object
            [ "role" A..= ("assistant" :: Text), "content" A..= (view amContent am ^. non "") ]
        SystemMessage _ -> Nothing  -- Extracted separately
        ToolResultMessage _ _ result _ -> Just
          $ A.object [ "role" A..= ("user" :: Text), "content" A..= result ]
    in
  ...
  let ( systemPrompt, anthropicMsgs ) = convertToAnthropicMessages messages
      ...
      body
        ...
        $ catMaybes
          ...
          , if null tools
              then Nothing
              else Just $ "tools" A..= convertToAnthropicTools tools
...
parseAnthropicResponse val = do
  ...
  let textBlocks = flip mapMaybe (toList content) $ \case
