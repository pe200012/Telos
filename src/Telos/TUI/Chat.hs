...
import           WEditor.LineWrap            ( BreakWords, breakExact )
...
-- | Chat message
data ChatMessage = ChatMessage { _messageText :: Text, _messageSender :: MessageSender }
...
data ChatState
  = ChatState { _chatHistory  :: [ ChatMessage ]
...
drawChatUI st = [ vBox [ historyWidget, inputWidget, statusBar ] ]
  where
    ...
    historyWidget
      = let
          ...
          borderStyle
            = if isFocused && st ^. currentMode == NormalMode
              then BS.unicodeBold
              else BS.unicode
        in 
    ...
    inputWidget
      = let
          ...
          editorTextAttr
            = if st ^. currentMode == InsertMode && st ^. focusedPanel == InputPanel
      ...
      = let
        ...
        in 
          ...
          $ hBox
            ...
            , txt $ "Focus: " <> focusText
            , txt " | "
            , txt "Ctrl+Enter: Submit | Ctrl+D: Quit"
            ]
