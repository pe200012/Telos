...
renderConfigError = \case
    ConfigNotFound path ->
        "Config file not found: " <> toText path
    ConfigParseError path err ->
        "Failed to parse config at " <> toText path <> ": " <> err
    ConfigValidationError err ->
        "Config validation error: " <> err
...
findProjectConfig = do
  ...
  where
    ...
    findUp dir = do
        found <- findFirst (map (dir </>) configNames)
        case found of
...
mergeConfigs base override = TelosConfig
    { _tcModel            = override ^. tcModel
    , _tcSmallModel       = override ^. tcSmallModel <|> base ^. tcSmallModel
    , _tcProviders        = Map.unionWith mergeProvider 
                                (base ^. tcProviders) 
                                (override ^. tcProviders)
  ...
  where
    ...
    mergeProvider b o = ProviderConfig
        { _pcApiKey  = o ^. pcApiKey <|> b ^. pcApiKey
...
applyEnvOverrides cfg = do
    ...
    pure $ cfg
        ...
        & maybe id (\l -> tcLogLevel .~ l) (mLogLevel >>= logLevelFromText . toText)
        ...
        & maybe id (\n -> tcMaxIterations .~ n) (mMaxIter >>= readMaybe)
  where
    setProviderKey :: Text -> Text -> TelosConfig -> TelosConfig
    setProviderKey name key = tcProviders %~ Map.alter (Just . updateKey) name
      where
        updateKey Nothing = defaultProviderConfig { _pcApiKey = Just key }
        updateKey (Just p) = p { _pcApiKey = Just key }
