...
instance FromJSON Permission where
  parseJSON = withText "Permission" $ \t -> case permissionFromText t of
    Just p  -> pure p
...
instance FromJSON LogLevel where
  parseJSON = withText "LogLevel" $ \t -> case logLevelFromText t of
    Just l  -> pure l
...
data ProviderConfig
  = ProviderConfig { _pcApiKey  :: Maybe Text  -- ^ API key (can use ${ENV_VAR} syntax)
...
defaultProviderConfig
  = ProviderConfig { _pcApiKey = Nothing, _pcBaseURL = Nothing, _pcTimeout = Nothing }

instance FromJSON ProviderConfig where
  parseJSON = withObject "ProviderConfig" $ \o
    -> ProviderConfig <$> o .:? "api_key" <*> o .:? "base_url" <*> o .:? "timeout"
...
instance FromJSON McpConfig where
  parseJSON = withObject "McpConfig" $ \o -> do
    ...
    timeout <- o .:? "timeout"
    case mcType of
      "local"  -> McpLocal name <$> o .: "command"
        <*> o .:? "args" .!= []
        <*> o .:? "env" .!= Map.empty
        <*> o .:? "work_dir"
        <*> pure enabled
        <*> pure timeout
      "remote" -> McpRemote name <$> o .: "url"
        <*> o .:? "headers" .!= Map.empty
        <*> pure enabled
        <*> pure timeout
...
instance ToJSON McpConfig where
  toJSON (McpLocal name cmd args env workDir enabled timeout)
    = object
    $ catMaybes
      [ Just $ "name" .= name
      ...
      , if enabled
          then Nothing
          else Just $ "enabled" .= enabled
      , ("timeout" .=) <$> timeout
      ]
  toJSON (McpRemote name url headers enabled timeout)
    = object
    $ catMaybes
      [ Just $ "name" .= name
...
data CompactionConfig
  = CompactionConfig { _ccAuto      :: Bool    -- ^ Automatically compact when threshold reached
...
defaultCompactionConfig = CompactionConfig { _ccAuto = True, _ccPrune = True, _ccThreshold = 0.8 }

instance FromJSON CompactionConfig where
  parseJSON = withObject "CompactionConfig" $ \o -> CompactionConfig <$> o .:? "auto" .!= True
    <*> o .:? "prune" .!= True
    <*> o .:? "threshold" .!= 0.8

instance ToJSON CompactionConfig where
  toJSON CompactionConfig { .. }
    = object [ "auto" .= _ccAuto, "prune" .= _ccPrune, "threshold" .= _ccThreshold ]
...
instance ToJSON TelosConfig where
  toJSON TelosConfig { .. }
    ...
    $ catMaybes
      ...
      , if Map.null _tcProviders
          then Nothing
          else Just $ "providers" .= _tcProviders
      , if Map.null _tcMcp
          then Nothing
          else Just $ "mcp" .= mcpToObject _tcMcp
      , if Map.null _tcPermissions
          then Nothing
          else Just $ "permissions" .= _tcPermissions
      , if null _tcInstructions
          then Nothing
          else Just $ "instructions" .= _tcInstructions
    ...
    where
      ...
      mcpToObject m
        = Object $ KM.fromList [ ( Key.fromText k, mcpToValue v ) | ( k, v ) <- Map.toList m ]

      mcpToValue :: McpConfig -> Value
      mcpToValue (McpLocal _ cmd args env workDir enabled timeout)
        = object
        $ catMaybes
          [ Just $ "type" .= ("local" :: Text)
          , Just $ "command" .= cmd
      ...
      mcpToValue (McpRemote _ url headers enabled timeout)
        = object
        $ catMaybes
          [ Just $ "type" .= ("remote" :: Text)
          , Just $ "url" .= url
