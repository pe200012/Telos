...
import           Telos.CLI.Config       ( McpServerEntry, mseArgs, mseCommand
                                        , mseEnv, mseName, mseWorkDir )
...
data LazyServerManager = LazyServerManager
  { _lsmServers :: TVar (Map Text ServerState)
  }
...
registerServer mgr entry = atomically $ do
  STM.modifyTVar' (mgr ^. lsmServers) $ 
    Map.insert (entry ^. mseName) (StateRegistered entry)

-- | Convert McpServerEntry to ServerConfig
toServerConfig :: McpServerEntry -> ServerConfig
toServerConfig entry = makeServerConfig 
    (entry ^. mseName) 
    (entry ^. mseCommand) 
    (entry ^. mseArgs)
...
getOrConnectServer mgr serverName = do
  ...
  case serverState of
    ...
    Just (StateFailed _ err) -> pure $ Left $ MCPConnectionFailed $ "Server previously failed: " <> err
    Just (StateRegistered entry) -> do
      ...
      case result of
        Left err -> do
          atomically $ STM.modifyTVar' (mgr ^. lsmServers) $
...
getServerStatus mgr = do
  servers <- atomically $ readTVar (mgr ^. lsmServers)
      ...
      case connResult of
        ...
        Just conn -> do
          ...
          case toolsResult of
            ...
            Right ltr -> 
              if any (\ti -> (ti ^. tiName) == toolName) (ltr ^. ltrTools)
  ...
  results <- forM (Map.toList servers) $ \(name, serverState) -> do
    ...
    case connResult of
      ...
      Right conn -> do
        ...
        case toolsResult of
          Left err -> pure $ Left err
          Right ltr -> pure $ Right $ map (\ti -> 
            let tool = makeTool (ti ^. tiName) (ti ^. tiInputSchema)
...
shutdownAllLazy mgr = do
  ...
  forM_ (Map.elems servers) $ \serverState ->
    case serverState of
