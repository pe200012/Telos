...
makeServerConfig name cmd args
  = ServerConfig
  { _scName = name, _scCommand = cmd, _scArgs = args, _scWorkDir = Nothing, _scEnv = Nothing }

data ClientInfo = ClientInfo { _ciName :: Text, _ciVersion :: Text }
  deriving stock ( Eq, Show, Generic )

makeLenses ''ClientInfo

makeClientInfo :: Text -> Text -> ClientInfo
makeClientInfo name version = ClientInfo { _ciName = name, _ciVersion = version }

instance ToJSON ClientInfo where
  toJSON ci = object [ "name" .= (ci ^. ciName), "version" .= (ci ^. ciVersion) ]

data ServerInfo = ServerInfo { _siName :: Text, _siVersion :: Maybe Text }
  deriving stock ( Eq, Show, Generic )
...
newtype RootsCapability = RootsCapability { _rcListChanged :: Maybe Bool }
  deriving stock ( Eq, Show, Generic )
...
data ClientCapabilities
  = ClientCapabilities
  { _ccRoots :: Maybe RootsCapability, _ccSampling :: Maybe SamplingCapability }
  deriving stock ( Eq, Show, Generic )

makeLenses ''ClientCapabilities

makeClientCapabilities :: ClientCapabilities
makeClientCapabilities = ClientCapabilities { _ccRoots = Nothing, _ccSampling = Nothing }
...
newtype PromptsCapability = PromptsCapability { _pcListChanged :: Maybe Bool }
  deriving stock ( Eq, Show, Generic )
...
data ResourcesCapability
  = ResourcesCapability { _rscSubscribe :: Maybe Bool, _rscListChanged :: Maybe Bool }
  deriving stock ( Eq, Show, Generic )
...
makeResourcesCapability
  = ResourcesCapability { _rscSubscribe = Nothing, _rscListChanged = Nothing }

newtype ToolsCapability = ToolsCapability { _toolsListChanged :: Maybe Bool }
  deriving stock ( Eq, Show, Generic )
...
data ServerCapabilities
  = ServerCapabilities { _scapLogging   :: Maybe Bool
...
data InitializeParams
  = InitializeParams { _ipProtocolVersion :: ProtocolVersion
...
makeInitializeParams info
  = InitializeParams { _ipProtocolVersion = currentProtocolVersion
...
data InitializeResult
  = InitializeResult { _irProtocolVersion :: ProtocolVersion
...
data ToolInfo = ToolInfo { _tiName :: Text, _tiDescription :: Maybe Text, _tiInputSchema :: Value }
  deriving stock ( Eq, Show, Generic )

makeLenses ''ToolInfo

makeToolInfo :: Text -> Value -> ToolInfo
makeToolInfo name schema
  = ToolInfo { _tiName = name, _tiDescription = Nothing, _tiInputSchema = schema }

instance FromJSON ToolInfo where
  parseJSON = withObject "ToolInfo" $ \o
    -> ToolInfo <$> o .: "name" <*> o .:? "description" <*> o .: "inputSchema"

data ListToolsResult = ListToolsResult { _ltrTools :: [ ToolInfo ], _ltrNextCursor :: Maybe Text }
  deriving stock ( Eq, Show, Generic )
...
data CallToolParams = CallToolParams { _ctpName :: Text, _ctpArguments :: Maybe Value }
  deriving stock ( Eq, Show, Generic )

makeLenses ''CallToolParams

makeCallToolParams :: Text -> Maybe Value -> CallToolParams
makeCallToolParams name args = CallToolParams { _ctpName = name, _ctpArguments = args }
...
data CallToolResult = CallToolResult { _ctrContent :: [ ContentPart ], _ctrIsError :: Maybe Bool }
  deriving stock ( Eq, Show, Generic )
...
data ResourceInfo
  = ResourceInfo
  { _riUri :: Text, _riName :: Text, _riDescription :: Maybe Text, _riMimeType :: Maybe Text }
  deriving stock ( Eq, Show, Generic )
...
data ListResourcesResult
  = ListResourcesResult { _lrrResources :: [ ResourceInfo ], _lrrNextCursor :: Maybe Text }
  deriving stock ( Eq, Show, Generic )
...
newtype ReadResourceParams = ReadResourceParams { _rrpUri :: Text }
  deriving stock ( Eq, Show, Generic )
...
data ResourceContents
  = ResourceContents
  { _resUri :: Text, _resMimeType :: Maybe Text, _resText :: Maybe Text, _resBlob :: Maybe Text }
  deriving stock ( Eq, Show, Generic )
...
newtype ReadResourceResult = ReadResourceResult { _rrrContents :: [ ResourceContents ] }
  deriving stock ( Eq, Show, Generic )
