...
import           Telos.Effect.MCP    ( ContentItem(..), MCP(..), ToolResult, makeToolResult )
...
runMockLLM
  :: Member (Embed IO) r => IORef (Either LLMError AssistantMessage) -> InterpreterFor LLM r
...
runTest :: IORef (Either LLMError AssistantMessage)
        -> IORef (Either MCPError ToolResult)
        -> Sem '[ LLM, MCP, Logger, Error AppError, Embed IO ] a
        -> IO (Either AppError a)
runTest llmRef mcpRef action
  = runM $ runError @AppError $ runNoopLogger $ runMockMCP mcpRef $ runMockLLM llmRef action
  ...
  describe "AgentLoop" $ do
    ...
    describe "mcpToolToCoreTool" $ do
      it "converts MCP ToolInfo to Core Tool with server prefix" $ do
        let mcpTool
              = makeToolInfo "read_file" (object [ "type" .= ("object" :: Text) ])
              & tiDescription ?~ "Reads a file"
