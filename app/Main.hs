...
import           Telos.Agent.Config        ( makeAgentConfig, acMaxIterations, acSystemPrompt )
import           Telos.Agent.Context       ( AgentContext, newAgentContext, registerTools
                                           , clearHistory )
import           Telos.Agent.Loop          ( AgentResult(..), runAgentLoop )
import           Telos.CLI.Config          ( CliConfig, ccModel, ccMcpServers, ccMaxIterations
                                           , ccSystemPrompt, configFilePath, loadConfig
                                           , mseName, mseCommand, mseArgs
                                           , mseWorkDir, mseEnv )
import           Telos.Core.Error          ( AppError )
import           Telos.Core.Types          ( Tool, toolName, toolDescription )
...
import           Telos.MCP.ServerManager   ( ServerManager, newServerManager, addServer
                                           , shutdownAll, aggregateTools, twsTool )
import           Telos.MCP.Types           ( makeServerConfig, scWorkDir, scEnv )
...
main = do
  ...
  forM_ (config ^. ccMcpServers) $ \entry -> do
    let serverCfg = makeServerConfig (entry ^. mseName) (entry ^. mseCommand) (entry ^. mseArgs)
  ...
  let tools = map (^. twsTool) $ either (const []) id toolsResult
  ...
  where
    loop = do
      ...
      if | cmd `elem` ["/quit", "/q", "/exit"] -> do
         ...
         | cmd == "/tools" -> do
             toolsResult <- aggregateTools serverMgr
             handleToolsCommand $ map (^. twsTool) $ either (const []) id toolsResult
             loop
